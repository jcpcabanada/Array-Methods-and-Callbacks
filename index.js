...
function getFinals(array) {
   const finalStage = fifaData.filter(function(item){
...
function getYears(array, getFinalsCb) {
    const yearsInFinals = getFinalsCb(array).map(function(item){
        return item.Year;
    });
    return yearsInFinals;
}
...
function getWinnersByYear(array, getYearsCb, getWinnersCb) {
    const champ = getWinnersCb(array, getFinals);
...
function getAverageGoals(array) {
   const aveHomeGoals = array.reduce(function(acc, item){
        return acc + item['Home Team Goals'] + item['Away Team Goals']
   }, 0);
   return (aveHomeGoals / array.length).toFixed(2);
}
